        ...
        >
            ...
            >
                ...
                <el-form-item label="评估流程" prop="postCode">
                    <el-input
                        v-model="chooseType.assessProcess"
                        :readonly="true"
                    />
                </el-form-item>
                <el-form-item label="评估人" prop="evaluatorId">
                    <el-select
                        ...
                        v-model="createMissionForm.evaluatorId"
                        placeholder=""
                        style="width: 100%"
                    >
                        <el-option
                            v-for="(item,index) in chooseType.evaluatorList"
                            :key="index"
                ...
                <el-form-item label="创建方式" prop="evaluatorIds">
                    <el-input
                        value="选择访视创建"
                        placeholder="请输入评估流程"
                        :readonly="true"
                ...
                <el-form-item label="包含的访视" prop="postCode">
                    <el-select
                        v-model="createMissionForm.visitIds"
                        multiple
                        style="width: 100%"
                    >
                        <el-option
                            v-for="item in chooseEvaluatorVisitList"
                            :key="item.visitId"
                            :label="item.visitName"
                            :value="String(item.visitId)"
                        />
                    </el-select>
                </el-form-item>
            </el-form>
...
<script>
import {
    listJobGlobalList as listPost,
    getPost,
    delPost,
    assessJobGlobalAdd,
    updatePost,
    getCreateAssessGlobalDetails,
    getAssessDataFallbackDetails,
    setAssessDataFallback,
} from "@/api/assess/task";
import report from "@/assets/images/report.png";
import EvaluatorDialog from "@/views/assess/independent/components/EvaluatorDialog.vue";
import Withdraw from "@/views/assess/independent/components/Withdraw.vue";
...
export default {
    ...
    methods: {
        ...
        changeType(v) {
            // 清空下方数据
            this.createMissionForm.evaluatorId = ''
            ...
            if (
                ...
                this.chooseType = this.creatNeedInfo.assessCfgList.find(
                    (item) => item.id == v
                );
            }
        },
        ...
        changeEvaluator(v){
            ...
            this.createMissionForm.visitIds = []
            if (
                this.chooseType.evaluatorList &&
                this.chooseType.evaluatorList.length > 0
            ) {
                this.chooseEvaluatorVisitList = this.chooseType.evaluatorList.find(
                    (item) => item.evaluatorId == v
                ).visitList
            }
            console.log(66666,this.chooseEvaluatorVisitList)
        ...
        getCreateAssessDetails() {
            getCreateAssessGlobalDetails({
                ...
                // if(this.creatNeedInfo.assessCfgList && this.creatNeedInfo.assessCfgList.length==1){
                //   this.createMissionForm.assessCfgId = this.creatNeedInfo.assessCfgList[0].id
                // }
            });
        },
        /** 修改按钮操作 */
            ...
            this.$refs["createMissionForm"].validate((valid) => {
                if (valid) {
                    if (this.form.postId != undefined) {
                        ...
                        this.createMissionForm.projectId = JSON.parse(
                            localStorage["chooseProject"]
                        ).id;
                        this.createMissionForm.examineeId =
                            this.chooseNowItem.examineeId;
                        if (this.createMissionForm.visitIds < 1) {
                            return this.$message.warning("请至少选择一个访视");
                        }
                        this.createMissionForm.visitIds = this.createMissionForm.visitIds.join(",");
                        assessJobGlobalAdd(this.createMissionForm).then(
                            (response) => {
                                this.$modal.msgSuccess("新增成功");
                                this.open = false;
                                this.createMissionForm.visitIds = this.createMissionForm.visitIds.split(",");
                                this.getList();
                            }
                        ).catch(err=>{
                            this.createMissionForm.visitIds = this.createMissionForm.visitIds.split(",");
                            console.log(6666,this.createMissionForm.visitIds)
                        })
                    }
                }
            });
