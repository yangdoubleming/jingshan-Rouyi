<template>
  <div>
    <el-button @click="showDialog">{{ type === 'create' ? '创建访视信息配置' : '修改' }}</el-button>

    <el-dialog
      v-loading="loading"
      :title="type === 'create' ? '创建访视信息' : '修改访视信息'"
    ...
    >
      <el-form ref="form" :model="form" label-width="120px">
        <el-form-item
          ...
          label="访视阶段"
          :rules="[
            { required: true, message: '访视阶段不能为空' }
          ]"
        >
        ...
        <el-form-item v-if="['4', '5'].includes(form.inputMode)" label="下拉框配置">
          <div
            v-for="(item, index) in form.downBoxValueList"
            :key="item"
          ...
          >
            ...
            <el-button
              v-if="index === form.downBoxValueList.length - 1"
              type="primary"
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" :disabled="inRequest" @click="handleSubmit">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
import { addVisitInfoCfg, editVisitInfoCfg, visitInfo } from '@/api/project/list'
export default {
  ...
  methods: {
    showDialog() {
      if (this.data) {
        this.loading = true
        visitInfo({ id: this.data.id, projectId: this.data.projectId }).then(res => {
          this.form = res.data
          if (this.form.downBoxValue) {
            this.form.downBoxValue = JSON.parse(this.form.downBoxValue)
            this.form.downBoxValueList = this.form.downBoxValue.map((item) => ({ value: item }))
          }else{
    ...
    handleSubmit() {
      if (this.type === 'create') {
        this.createVisitInfoCfg()
      }
      if (this.type === 'update') {
        this.updateVisitInfoCfg()
      }
    },
    async createVisitInfoCfg() {
      ...
      try {
        this.$refs['form'].validate(async(valid) => {
          if (valid) {
            ...
            this.inRequest = true
            const response = await addVisitInfoCfg({ ...this.form, projectId: this.projectId, fieldNameEn: '1', promptTextEn: 'please' })
      ...
      try {
        this.$refs['form'].validate(async(valid) => {
          if (valid) {
            if (this.inRequest) return
            this.inRequest = true
            const response = await editVisitInfoCfg({ ...this.form, projectId: this.projectId, fieldNameEn: '1', promptTextEn: 'please' })
            if (response.code === 200) {
              this.dialogVisible = false
              this.$message.success(response.msg)
              this.form = {
