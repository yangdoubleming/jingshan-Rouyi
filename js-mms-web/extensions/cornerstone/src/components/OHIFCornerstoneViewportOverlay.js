...
class OHIFCornerstoneViewportOverlay extends PureComponent {
  ...
  render() {
    ...
    } WL: ${windowWidth.toFixed ? windowCenter.toFixed(0) : windowCenter}`;
    ...
    const normal = (
      <React.Fragment>
        <div className="top-left overlay-element">
          ...
          <div>{modality}</div>
          <div>
            ST: {formatNumberPrecision(sliceThickness, 2)}mm SL:{' '}
            {formatNumberPrecision(sliceLocation, 2)}mm
          </div>
          <div>
            S: {seriesNumber >= 0 ? `Ser: ${seriesNumber}` : ''} I: {imageIndex}
            / {stackSize}
          </div>
          <div>未保存</div>
        </div>
        <div className="top-right overlay-element">
          {/* <div>{studyDescription}</div>Slice Location
          <div>
          ...
          {reconstructionDiameter ? (
            <div>
              ...
              {formatNumberPrecision(reconstructionDiameter, 2)}
            </div>
          ) : null}
          {xRay ? <div>X Ray Tube Current: {xRay}</div> : null}
          {kvp ? <div>KVP: {kvp}</div> : null}
        </div>
